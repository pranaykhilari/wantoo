...
/***/ function(module, exports, __webpack_require__) {
		...
		function stupidCounter(el) {
			...
			$.each(inputs, function (index, item) {
				if ($(item).attr('maxlength') !== undefined) {
					...
					$('<p class="-stupid-counter">' + currentCount + '</p>').insertBefore(item);
	 ...
	 */
		...
		function isMobile() {
			...
			})(navigator.userAgent || navigator.vendor || window.opera); // jshint ignore:line
			return check;
		}
	 ...
	 */
		...
		voteButton.on('click', function (event) {
			...
			if (isUserAuthed()) {
				...
				if ($(self).hasClass('not_voted')) {
					...
					(0, _tracking.track)('vote', {
						'source': 'idea detail'
					});
				} else if ($(self).hasClass('voted')) {
					addVote = false;

					(0, _tracking.track)('unvote', {
						'source': 'idea detail'
					});
				}
		...
		yourIdea.bind('paste', function (e) {
			...
			e.preventDefault();
			if (firstFocus) {
				(0, _tracking.track)('add idea focused');
			...
			var content = ($(this).text() + e.originalEvent.clipboardData.getData('text')).slice(0, 70);
		...
		addIdeaAddButton.on('click', function () {
			...
			(0, _tracking.track)('add idea button clicked');

			if ($(self).hasClass('_step-1')) {

				ideaInfo.text = $('._yourIdea').text();
	 ...
	 */
		...
		ideaDescriptionSave.on('click', function () {
			...
			var text = $('._ideaDescriptionInput').val();
		...
		sendMessage.on('click', function () {
			...
			(0, _tracking.track)('comment added');
		});
		...
		$('#submit_model_login').click(function (e) {
			...
			if (check >= 2) $('#signup_form').submit();else {
	 ...
	 */
		...
		$('._sendMessage').on('click', function () {
			...
			if (isUserAuthed()) {
				var csrftoken = getCookie('csrftoken');
				var commentStr = $('._comment-text').val();
				$('._comment-text').val('');
		...
		searchInput.on('keyup', function () {
			if (firstKeyUp) {
				(0, _tracking.track)('search input focused');
				firstKeyUp = false;
			}
		});
		...
		filterOption.on('click', function () {
			(0, _tracking.track)('filter option clicked');
		});

		var shareOption = $('._shareOption');

		shareOption.on('click', function () {
			(0, _tracking.track)('share option clicked', {
				'source': 'idea detail'
			});
		});
		...
		addIdeaNavBtn.on('click', function () {
			...
			(0, _tracking.track)('add idea in main nav clicked');

			if (windowWidth > 650) {
				window.location.pathname = '/' + window.userInfo.company + '/';
			} else {
				window.location.pathname = '/' + window.userInfo.company + '/add-idea/';
			}
		});
		...
		$('#sendProEmail').on('click', function () {
			(0, _tracking.track)('Pro Request Button clicked on modal');
		...
		function addKeyword(word) {
			var keyword = '<span class="keyword"><span class="keyword-wrap">' + '<span class="key-close">X</span>' + word.trim() + '</span></span>';
		...
		if ($('.twitter-search__input').length) {
			...
			var inputs;
			if (localStorage.length > 0) inputs = localStorage.getItem('wantoo_twitter_keywords');else inputs = $('.twitter-search__input').val();
			inputs = inputs.split(' ');
			$('.twitter-search__input').val('');
			for (var i = 0; i < inputs.length; i++) {
				if (inputs[i].trim().length > 0) addKeyword(inputs[i].trim());
			}
		}
